<?php
	...
	class MySqlSelectQueryBuilderBehavior extends UnitTestCase
	{
		...
		public function test_build_shouldHandleEmptyQuery()
		{
			...
			$this->assertEqual($result, "SELECT * FROM table");
		}
		...
		public function test_build_shouldHandleSingleSelect()
		{
			...
			$this->assertEqual($result, "SELECT col FROM table");
		}
		...
		public function test_build_shouldHandleMultipleSelects()
		{
			...
			$this->assertEqual($result, "SELECT col1,col2 FROM table");
		}
		
		public function test_build_shouldHandleSingleWhereWithNoSelect()
		{
			$result = $this->_builder->where(array("col='1'"))->buildFor("table");
			
			$this->assertEqual($result, "SELECT * FROM table WHERE col='1'");
		}
		
		public function test_build_shouldHandleSingleWhereWithWhere()
		{
			$result = $this->_builder->select(array("col"))->where(array("col='1'"))->buildFor("table");
			
			$this->assertEqual($result, "SELECT col FROM table WHERE col='1'");
		}
		...
		public function test_build_shouldHandleMultipleWheres()
		{
			...
			$this->assertEqual($result, "SELECT col1,col2 FROM table WHERE col1='1',col2='2'");
		}
	}
